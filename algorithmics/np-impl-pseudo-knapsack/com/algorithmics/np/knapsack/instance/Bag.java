...
public class Bag implements Certificate {
    ...
    public List<Item> getContents() {
        return isItemPicked.keySet().stream().filter(isItemPicked::get)
    ...
    public int getValue() {
        return isItemPicked.keySet().stream().filter(isItemPicked::get).map(Item::getValue)
                .reduce(0, (i1, i2) -> i1 + i2);
    }

    @Override
    public String toString() {
        String result = "picked items:\n";
        for (Item item : items) {
            if (isItemPicked.get(item)) {
