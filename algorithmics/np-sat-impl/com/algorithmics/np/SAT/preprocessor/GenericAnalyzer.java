...
import com.algorithmics.np.SAT.instance.tree.BooleanOperation;
...
public class GenericAnalyzer implements IGenericAnalyzer<SentenceTree> {
	...
	public enum NODE_CLASS{AAndSentence,AOrSentence,AVarSentence,ANotvarSentence,ANotsentenceSentence};
	...
	public SentenceTree caseAAndSentence(AAndSentence node) {
		SentenceTree st=SentenceTree.constructNonLeafTree(BooleanOperation.AND, apply(node.getLeft()), apply(node.getRight()));
		return st;
	}

	@Override
	public SentenceTree caseAOrSentence(AOrSentence node) {
		SentenceTree st=SentenceTree.constructNonLeafTree(BooleanOperation.OR, apply(node.getLeft()), apply(node.getRight()));
		return st;
	}
	...
	public SentenceTree caseTLpar(TLpar node) {
		throw new RuntimeException(String.format("Internal Error: Concrete Syntax Tree %s is supposedly ignored", node.getText()));
