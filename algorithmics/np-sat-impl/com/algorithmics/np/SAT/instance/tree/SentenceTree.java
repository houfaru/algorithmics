...
public class SentenceTree extends Sentence{
	private Optional<BooleanOperation> operation;
	...
	public static SentenceTree constructNonLeafTree(BooleanOperation op,SentenceTree firstTree,SentenceTree secondTree, SentenceTree ...otherTrees) {
	...
	public HashSet<Variable> getVariables() {
		...
		if(isLeaf()) {
			if(!var.isPresent()) {
				throw new RuntimeException("Internal Error, a sentence tree leaf do not contain variable");
			}
	...
	public boolean verify(VariableAssignment certificate) {
		if(isLeaf()) {
			...
			if(operation.get()==BooleanOperation.AND) {
				boolean res=true;
				for(SentenceTree subTree:subTrees.get()) {
			...
			if(operation.get()==BooleanOperation.OR) {
				boolean res=false;
		...
		throw new RuntimeException("Internal Error: undefined boolean Operation on nonleaf node : "+operation.get());
	}
	@Override
	public String toString() {
		if(isLeaf()) {
			...
			if(!operation.isPresent()) {
				throw new RuntimeException("Internal Error: undefined boolean Operation on nonleaf node");
			}
			if(!subTrees.isPresent()) {
				throw new RuntimeException("Internal Error: subtrees undefined on nonleaf node");
			}
			String operatorSymbol=operation.get().toString().equals("AND")?Symbol.AND+"":Symbol.OR+"";
	...
	public SentenceInCNF toCNF() {
		...
		if(this.operation.get().equals(BooleanOperation.OR) && this.getDepth()==1) {
		...
		if(this.operation.get().equals(BooleanOperation.AND)) {
		...
		if(this.operation.get().equals(BooleanOperation.OR)) {
	...
	public void flipOperation() {
		...
		if(this.operation.get().equals(BooleanOperation.AND)) {
			this.operation=Optional.of(BooleanOperation.OR);
		}else {
			this.operation=Optional.of(BooleanOperation.AND);
		}
	}
